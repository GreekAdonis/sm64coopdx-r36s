...
#ifdef PROFILE_BUILD
// Tag the call so smlua_call_hook() can attribute its time to a hook type. The
// dispatchers are autogenerated, so labelling them here rather than inside them
// keeps this out of the generator entirely. The comma expression still yields
// the dispatcher's bool, so call sites that test the result are unaffected.
extern u16 gProfileCurHookType;
#define smlua_call_event_hooks(hookEventType, ...) \
    (gProfileCurHookType = (hookEventType), smlua_call_event_hooks_##hookEventType(__VA_ARGS__))
#else
#define smlua_call_event_hooks(hookEventType, ...) \
